...
import { getAccount, setAccount, clearAllData, getRecoveryVault, getEncryptedVault, setEncryptedVault, setRecoveryVault, setStatus, setDerivedKey, setConfig, getConfig } from "@/lib/storage";
  ...
  const handleImport = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        // Store recovery vault if present
        if (backup.recoveryVault) {
          await setRecoveryVault(backup.recoveryVault);
        }

        // Set wallet as locked (user must enter password to unlock)
        await setStatus("locked");

        // Restore tier/license if present
